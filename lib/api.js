...
import { CATALOGUE, CROPS, DISTRICTS, districtZone } from './catalogue.js';
  ...
  'POST /api/listings': (ctx) => {
    ...
    const r = run(
      `INSERT INTO listings (seller_id,crop_key,cat,title,note,price,unit,qty,negotiable,organic,
        harvested,photo_id,district,village,created_at,updated_at)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      ...
      String(b.harvested || '').slice(0, 40), b.photoId || null,
...
function shapeThread(t, viewer, withMessages) {
  ...
  const out = {
    ...
    listing: listing ? {
      ...
      photoId: listing.photo_id, icon: crop.icon || '🧺', village: listing.village,
